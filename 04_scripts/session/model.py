...
K0 = np.pi / 2
# J-sector on-site well (the dimer nodes). 'radial' (default since 2026-09-26):
# per dimer psi_j = u_{2j} + i u_{2j+1}, V_j = sqrt5 r^2/2 + r^3/3 with r = |psi_j|,
# F_j = -(sqrt5 + r) psi_j -- invariant under the phase rotation J generates, so
# phase charge is conserved at every order; bounded motion at every energy; the
# linear part is unchanged (MODEL_SPEC sec 1, "The node form"). 'elementwise': the
# earlier per-component square -(sqrt5 u + u*u), kept so results before 2026-09-26
...
class Lattice:
    ...
    def __init__(self, n, N=200, kappa=KAPPA, gyro_scalar=None,
                 C_r=0.0, tower=None, q=1, shape=None, well=None):
        ...
        # on-site nonlinearity: radial per dimer in the J sector, per component
        # in the scalar sector (see J_WELL)
        self.well = "elementwise" if self.beta else (well or J_WELL)
    ...
    def force(self, u, v, W=None, Wm=None):
        ...
        f = -(SQ5 * u + self._onsite_nl(u)) + C * self._lap(u)
        if self.kappa:
    ...
    def energy(self, u, v):
        ...
        return (0.5 * (v * v).sum() + 0.5 * SQ5 * (u * u).sum()
                + self._onsite_cubic(u) + 0.5 * C * grad)
...
def free_Aeff(lat, width, n0, amp, T, dt=1.0):
    """A_eff(t) = sum F^3 / sum F^2 for the launched packet's envelope F = |psi|
    under LINEAR free evolution (exact, per mode on the a-branch), t = 0..T."""
    x = np.arange(lat.N)
    psi = amp * np.exp(-0.5 * ((x - n0) / width) ** 2) * np.exp(1j * K0 * (x - n0))
    P = np.fft.fft(psi)
    q = 2 * np.pi * np.fft.fftfreq(lat.N)
    w = 0.5 * (-lat.kappa + np.sqrt(lat.kappa ** 2 + 4 * (SQ5 + 2 * C * (1 - np.cos(q)))))
...
def precession_prediction(lat, seq, T, width, n0, amp, psi0):
    """Independent prediction for the ordering test, with the radial well's
    FIRST-ORDER SELF-PRECESSION (derivation: MODEL_SPEC sec 4d, "Gate 7 under the
    radial well"). A circular a-wave in dimer j sees stiffness sqrt5 + |psi_j|, so its
    local frequency shifts by |psi_j| / (2w + kappa). For psi_j(n, t) = s_j F(n, t) the
    lattice-summed density matrix acquires, to first order, the diagonal rotation
        P = diag( exp(-i |s_j| I / (2w + kappa)) ),   I = integral of A_eff dt,
    A_eff = sum F^3 / sum F^2 from the linear free envelope. Segments are applied as
    unitaries at their centre-crossing times t_s = (start + len(RAMP)/2 - n0)/v_g.
    Neglected: the O(A g) change of the in-segment wavenumbers, O(A^2), reflections.
    With the elementwise well the self-precession is second order: plain product."""
    s = psi0.astype(complex)
    if lat.well != "radial":
        for start, ax, g in seq:
            s = U_segment(lat, ax, g) @ s
        return coords_of_state(lat, s)
    vg = 2 * C * np.sin(K0) / (2 * lat.omega + lat.kappa)
    ts, Ae = free_Aeff(lat, width, n0, amp, T)

    def I(a, b):
        m = (ts >= a) & (ts <= b)
        return np.trapezoid(Ae[m], ts[m]) if m.sum() > 1 else 0.0

    den = 2 * lat.omega + lat.kappa
    t = 0.0
    for start, ax, g in seq:
        tc = (start + len(RAMP) / 2 - n0) / vg
        s = np.exp(-1j * np.abs(s) * I(t, tc) / den) * s
        s = U_segment(lat, ax, g) @ s
        t = tc
    s = np.exp(-1j * np.abs(s) * I(t, T) / den) * s
    return coords_of_state(lat, s)
...
def _windows(lat, u, v, seg):
    ...
    idx = np.arange(lat.N)
    return [float(w[(idx >= s0 - 10) & (idx < s0 + len(RAMP) + 10)].sum() / w.sum()) for s0 in seg]


def clear_time(lat, spec, seg, width, n0, amp, thr=1e-6):
    """First time every segment window holds < thr of the packet weight."""
    W, Wm = make_links(lat, spec)
    u, v = lat.packet(width=width, n0=n0, amp=amp, per_mode=True)
    t, seen = 0.0, False
    while t < 4000:
        u, v, _ = lat.run(u, v, 5.0, W, Wm)
        t += 5.0
        fr = _windows(lat, u, v, seg)
        seen = seen or max(fr) > 1e-3
        if seen and max(fr) < thr:
            return t
    raise RuntimeError("packet never cleared the segment windows")


def ordering_test(n, gA, gB, seg=(60, 80), T=180.0, q=1, side=None,
                  width=None, n0=None, axes=(0, 1), readout="fixed", N=None, amp=1e-3):
    """Two segments in both orders; compare against independent prediction.

    q = 1 (default): the verified chain protocol, segments (60, 80).
      readout="fixed": N = 200, read at the fixed T (180) -- the earlier protocol,
        which reads with the packet's tail still in the second window (MODEL_SPEC
        sec 4d.1, trap 6).
      readout="clear": N = 1200, both orders read at ONE common time, 1.1 x the later
        clearing time (every window < 1e-6), as shape_zero_tests/gate7_readout.py.
      The prediction includes the radial well's first-order self-precession
      (precession_prediction); with the elementwise well it is the plain product.
    ...
    """
    if q == 1:
        lat = Lattice(n=n, N=N or (1200 if readout == "clear" else 200))
        width = width or 8.0
    ...
    runs = (("AB", [(seg[0], a0, gA), (seg[1], a1, gB)], (1, 0)),
            ("BA", [(seg[0], a1, gB), (seg[1], a0, gA)], (0, 1)))
    if q == 1 and readout == "clear":
        T = 1.1 * max(clear_time(lat, sp, seg, width, n0, amp) for _, sp, _ in runs)
    out = {"T": T}
    for name, spec, order in runs:
        W, Wm = make_links(lat, spec)
        u, v = lat.packet(width=width, n0=n0, amp=amp, per_mode=True)
        ...
        if q == 1:
            ...
            pred = coords_of_state(lat, Upred @ psi0)
        out[name] = (co, pred, pur, drift)
    return out
...
def main():
    ...
    for nn, gA, gB, lbl in ((2, 0.12, 0.08, "u(2)"), (3, 0.15, 0.10, "u(3)")):
        ...
        # segment), so the ten-times test cannot fail. The meaningful test is the
        # split against the INDEPENDENT prediction. Under the radial well (default
        # since 2026-09-26) the prediction includes the first-order self-precession
        # and the floor is a PREDICTED nonzero value: it must match (|dfloor| <
        # 0.05 deg). With the fixed readout the old sanity test floor < 0.5 applies.
