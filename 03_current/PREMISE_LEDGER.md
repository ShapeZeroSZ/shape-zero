...
| P3 | **Persistence** | What endures is what exists to be studied | D1 periodicity; q ≤ 3 (C5); Euclidean signature (C6) | **principle** — used at D1 only in the formal proofs | `shape_zero_zero_ladder.md`, "The question and the method"; `C1S_SYNTHESIS.md` §5, §14 |
...
| C14 | **κ ≥ κ\* = 2c/√(K + 2c) = 0.971737** at c = 1, K = √5 (equal to 2φ^(−3/2) because 2 + √5 = φ³ — a consequence of K = √5, not a selection). A floor only: no principle supplies an upper side. At q = 3 the same floor applies because the model's slab segments conserve transverse momentum; a segment of finite transverse width would need 2qc/√(K + 2qc) = 2.091 | κ_req(k₀) = x/√(K + x), x = c(1 − cos k₀), maximal at the band edge; q = 3 from `make_links` and `jcompat_q3.py` (predictions committed first, 9bb9951) | P8, P9, K6, S1 | MODEL_SPEC §3, "CANDIDATE" and "ADOPTED"; INPUT_LEDGER §2d #3 |
...
| C27 | **P-3, spinor self-precession**: Ω = C·A²·n_z with n_z conserved; at κ\* C = −0.0491 predicted, −0.0495 measured (κ = 0.5: −0.0896 / −0.0899) | closed form; `phi_gauge_precession.py`, `shape_zero_tests/p3_kstar.py` | P9, S5, S6, K3, K6 | `01_source/shape_zero_predictions_v1.md`, P-3; MODEL_SPEC §3, "ADOPTED" |
...
| **J-compatibility beyond first order** | the second-order leftover in a closed channel: open | MODEL_SPEC §3, "UPDATE" |
