...
| P9 | **The φ-well force law** | Each node sits in the on-site force F = −(x² − x − 1): fixed points at the golden-ratio roots, linear stiffness √5, quadratic nonlinearity | MODEL_SPEC §1, every force law in `04_scripts/session/model.py` | **principle** — "stated … and not derived"; the source spec's universality control reports every measured dynamical law reproducing on a generic-cubic lattice (K = 2, α = 0.7) [**2026-09-26 — node form adopted:** at a dimer node (the J sector) the well acts on the dimer radius, F = −(√5 + \|ψ\|)ψ (form (A)); the scalar β sector keeps the per-component form. Chosen by the principles alone (§1d). The φ-well itself **remains an underived premise**] [**RESTATED 2026-09-26 (MODEL_SPEC §1b):** what remains a premise is **a quadratic on-site force, applied radially (form (A))**, F = −(K + \|ψ\|)ψ. Its **φ form is a coordinate choice**: every quadratic force with two real roots is unit-equivalent (x = y + ½, then rescaling y and t, gives −(z² − 1)), and the well contributes zero dimensionless parameters] | INPUT_LEDGER §2d, "Premise, not a parameter"; SCALE_SCOPING §1b; MODEL_SPEC §1; `shape_zero_v5-3.txt`, "Universality control and the status of the golden ratio (v5.3)" |
...
| K4 | **the a–b angle** | direction of D8's second generator | D8 | **chosen, kind 3** — the D8 frequency ratio sweeps [1.04, 23.9] [corrected 2026-09-26: 23.9 → 22.93, a resolution artifact; exactly 1/sin(θ/2), INPUT_LEDGER §3.2]; plurality forbids only b = ±a [**2026-09-26 — investigated, still chosen (kind 3):** the ratio is 1/sin(θ/2); orthogonality is not required (every θ in (0°, 180°) gives a 4-dimensional associative pair); the model contains no nonlinear perturbation of the D8 flow, so endurance cannot select an angle, and none was added; the golden angle stays "attainable, not selected". Misses: swept span 5, not 8; 𝕁 is neither a left nor a right octonionic multiplication (best-fit residual 0.82–1.00, post hoc fit). INPUT_LEDGER §3.2] | INPUT_LEDGER §2d #4, §3.2 |
...
| K9 | **K = √5** | the well's linear stiffness | every dispersion relation | part of **P9** (principle); stiffness varied only in tests [**2026-09-26:** a unit, not a parameter — it sets the amplitude and time units; the invariants are ĉ = c/K, κ̂ = κ/√K, β̂ = βc/√K, Ĉ_r = C_r/√K (MODEL_SPEC §1b)] (MODEL_SPEC §3, J-compatibility vs stiffness) | INPUT_LEDGER §2d, "Premise, not a parameter"; MODEL_SPEC §1, §3 |
...
| C35 | **The elementwise form violates persistence as bounded motion**: its per-component potential √5u²/2 + u³/3 is unbounded below past u = −√5, so a node escapes above energy 5√5/6 = 1.863 per component [**2026-09-26:** invariant form E/K³ = 1/6 for any quadratic well; 1.863 is 5√5/6 in the model's units] (a node with kinetic energy 5 escaped at t = 3.5) | derived; simulated | P3, P9 (elementwise) | MODEL_SPEC §1a; `shape_zero_tests/ring_spectrum.py` |
| C36 | **In the small-amplitude limit the radial model reduces to the linear gauge dynamics**: at q = 3 every deviation from the linear prediction extrapolates to zero within max(2σ, 0.01°); at q = 1 the floor does, and the split and per-order intercepts equal the single-carrier prediction's known error. All deviations are linear in amplitude — nothing beyond the first-order self-precession; the slopes are recorded as the target for its derivation | measured (3 amplitudes, predictions committed first); certification test | C9, C24, P9 (radial form) | MODEL_SPEC §4d, "Amplitude scaling"; `shape_zero_tests/amp_scaling.py`, `certify_gates.py` |
| C37 | **The slice-resolved first-order self-precession predicts gate 7's order-dependent floor at q = 1** (u(2) 0.16005 vs 0.16013°, u(3) 0.09624 vs 0.09642° per 10⁻³; the u(2) value was seen during setup, the u(3) value was not). It does **not** yet predict the per-order slopes (~10% high), q = 3 (10–30% off) or the split slopes (wrong sign for u(2)). Adopted as gate 7's floor prediction at q = 1 only | derived + measured (predictions committed first) | C36 | MODEL_SPEC §4d, "Slice-resolved self-precession"; `shape_zero_tests/slice_precession.py` |
| C38 | **The model has five independent dimensionless parameters and the well contributes none**: ĉ = c/K, κ̂ = κ/√K, β̂ = βc/√K, Ĉ_r = C_r/√K, θ_ab (\|b\|/\|a\| fixed at 1); amplitude A/K, ĝ = gc/√K, k₀, packet width, geometry, T√K, n and q are protocol settings. φ and √5 are coordinates in every adopted node form (in the ring form (B), not adopted, the root ratio −φ² would be physics) | derived (two free scalings: amplitude and time; the lattice spacing fixes length); checked numerically to 10⁻¹⁴ in four sectors | P9, K6, K9 | MODEL_SPEC §1b; `shape_zero_tests/scale_invariance.py` |
...
| **The D8 two-generator flow in the lattice** | not realised: 𝕁 is not an octonionic multiplication (best-fit residual 0.82–1.00 by L_x, R_y, L_x + R_y), the residual term supplies one generator (L_g), and κ and C_r have never been on together — θ_ab has no lattice counterpart | MODEL_SPEC §9; INPUT_LEDGER §3.2; `shape_zero_tests/d8_theta.py` |
