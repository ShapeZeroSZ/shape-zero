...
the repository supplies an upper side. [**CORRECTED 2026-09-25:** κ ≥ κ\* closes the
*linear* coupling channel at every wavelength; the model's own u∘u nonlinearity is
J-breaking and at κ\* converts a-waves near k = 0.49 and 1.25 into the opposite
chirality by second-harmonic resonance (confirmed by simulation, slope 0.991 × rA²),
so at κ\* J-compatibility holds at **linear order only** (MODEL_SPEC §3, "CORRECTION").]

**Finding, not a principle: persistence at every wavelength (2026-09-25).** Strict
persistence — no resonant channel for any wave — is unsatisfiable for all parameters
(same-branch four-wave quartets are always open). The strongest form the model admits,
...
**What fixes θ_ab? (investigated 2026-09-26; predictions committed first, 40abe6d; tests `shape_zero_tests/d8_theta.py`, `d8_theta_output.txt`, d420fa0).** **θ_ab remains a chosen genuine parameter (kind 3).** (1) **Orthogonality is not required:** every θ in (0°, 180°) gives a 4-dimensional associative (quaternion) pair — a, b generate a subalgebra of dimension 4 with associator ≤ 4×10⁻¹⁶ (2 only at 0°, 180°) — so a Fano line up to G₂ at every angle; missions 5 and 6 fix the table, not the generators; no D8 script or synthesis imposes a·b = 0; G₂ leaves θ the only invariant (spectra at equal θ agree to 2×10⁻¹⁵); the flow's octonionic content (swept span, nonzero associator ≈ 1.7–1.9) is the same at every θ in (0°, 180°); 90° (ratio √2) is not special. (2) **The model contains no nonlinear perturbation of the D8 flow**, so endurance cannot select an angle, and **none was added** (an added perturbation would put φ in rather than find it): every D8 law in the repository is linear in ψ, and the lattice does not carry the two-generator form. **The golden angle stays "attainable, not selected."** **Prediction misses:** the swept span of the generic flow is **5-dimensional, not 8** as predicted (θ-independent, as predicted); 𝕁 was predicted to be a left multiplication L_e — it is **neither a left nor a right octonionic (or sedenionic) multiplication**: the best fit of 𝕁 by L_x, R_y or L_x + R_y over all x, y leaves a relative residual of **0.82–1.00** (that extended fit is **post hoc**, added after the basis-unit comparison found no match).
