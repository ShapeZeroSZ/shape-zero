# Shape Zero C1S — Synthesis

Standalone account of what the C1S work established, across the whole ladder.
Sections 1–11 cover the octonionic rung; Sections 12–15 cover the ladder-wide
results (parameter census, scales, signature, Euclidean reading).
...
## 12. The parameter census

With ω identified as the unit, every remaining quantity is forced, a unit, or
free.

| rung | quantity | status |
|---|---|---|
| D1 | harmonic lattice ℤ | **forced** — Theorem 2.2 |
| D1 | ω | the **unit** |
...
| D4 | gyroscopic κ | **licensed** — plurality excludes κ = 0 |
| D8 | target metric scale | **forced** — 6 |
| D8 | the law | **forced** — plurality + minimality |
...
Schur settles *which* invariant structure; the embedding settles its size, and
both algebras are normalised by 1·1 = 1 with the composition law.

| structure | scale |
|---|---|
| D4 symplectic form | **fixed** — ‖[X,J₀]‖/‖X‖ = 2.000000, std 1.2e-16 |
| D8 target metric | **fixed** — ‖X·c‖/‖X‖ = 6.000000, std 5.8e-16 |
| D1 clock ω | **free** — f₀ ∝ √k with ratios invariant |

Both constants are integers counting algebra structure: 6 = 42/7 is the Fano
per-line multiplicity. The Kähler relation ω(u,v) = g(Ju,v) means fixing the D4
metric fixes its symplectic form too.

**ω cannot be fixed, and this is a proof rather than a gap.** Every input —
conservativity, minimality, persistence, plurality, existence — is dimensionless.
A dimensionless structure cannot produce a dimensionful output. So ω is not a
free parameter awaiting determination; it is **the unit**. Every ladder output is
correspondingly dimensionless: 1/2/3, 2, 6, 42, 16, 48, 14, 7→28, 22/8/14, 0-or-4.

## 14. Signature: the ladder cannot be Lorentzian

Hurwitz has two branches. Both are genuine composition algebras — the
multiplicative norm holds to 2.8e-14 in each.

| | quaternions | split-quaternions |
|---|---|---|
| signature | (4,0) | (2,2) |
| null vectors in 20,000 samples | **0** | **431** |
| level set of the conserved norm | **sphere**, compact | **hyperboloid**, non-compact |

**Persistence makes the choice.** Definite norm → compact level set → bounded
motion → Lemma 2.1 applies. Indefinite → orbits escape along null directions →
excluded. So the arenas are Euclidean at every rung, by consequence rather than
omission.
...
Euclidean signature is where derivations normally live — Euclidean field theory,
instantons, thermal physics all work there, with continuation to Lorentzian a
separate step. And periodic Euclidean time *is* finite temperature: the Matsubara
construction gives ω_n = 2πnT, the same form as D1's ℤ·ω.

**Verified:** the mode sum equals the closed thermal correlator to 5.7e-12, KMS
holds at 1.7e-16, and the β_T → ∞ limit reproduces the vacuum correlator exactly.
Euclidean periodicity is genuinely thermal, not merely periodic.

**But ω is NOT a temperature.** The identification requires the orbit to *be* a
thermal state, and it isn't. A single D1 orbit is **microcanonical**: its position
density is the arcsine law, peaked at the turning points (centre/edge = 0.29). The
canonical distribution is peaked at the centre (1.12) at every temperature; the
best fit runs to the scan boundary with an 84% residual. They are qualitatively
opposite and no β_T reconciles them.

What survives: **D1 derives the periodicity the thermal formulation imposes.**
Same lattice, opposite direction of explanation. That is the stronger position —
a theorem replacing an assumption — but it does not make ω a temperature.

**CORRECTED.** Calling the missing continuation "the ordinary gap every
Euclidean formulation has" was too generous. Euclidean QFT is Euclidean *by
choice*: the Lorentzian theory is primary, and the rotation goes both ways. The
ladder is Euclidean *by necessity*.

Wick rotation of the ladder's own algebra was tested directly — rotating two
imaginary units of ℍ sends e² = −1 to f² = +1, giving signature (2,2), 206 null
vectors in 20,000 samples, and a hyperboloid level set. **The rotation lands
exactly on the split branch persistence excluded.** There is no Lorentzian theory
to rotate back to; the rotation exits the ladder.

**So the ladder is not an incomplete Lorentzian theory. It is a complete
Euclidean one.** Persistence, boundedness, compactness, the integer lattice and
definite signature are a single package — remove any one and the rest go.

The cost is precise: **the ladder cannot describe propagation.** Null geodesics,
light cones, causal structure and lensing all require indefinite signature and
all lie outside it. What it can describe is bounded, recurrent, structural
content — spectra, algebras, orbits, invariants — which is what every solid
result in this package is.
